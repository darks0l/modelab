...
export class CampaignManager {
    ...
    createCampaign(params) {
        const id = `camp-${Date.now()}-${crypto.randomUUID().slice(0, 8)}`;
     ...
     */
    async forceSynthesize(campaignId) {
        const campaign = this.getCampaign(campaignId);
        if (!campaign)
            throw new Error(`Campaign not found: ${campaignId}`);
        const priorRuns = this.getCampaignRuns(campaignId);
    ...
    // ── Synthesis Engine ──────────────────────────────────────────────────────
    async synthesize(campaign, runLog, priorRuns, runNumber) {
    ...
    parseSynthesis(raw, best, worst, spread, campaign, runNumber, priorRuns) {
        try {
            ...
            return this.ruleBasedSynthesis(campaign, {}, priorRuns, runNumber, spread, best, worst);
        }
    }
    ruleBasedSynthesis(campaign, runLog, priorRuns, runNumber, spread, best, worst) {
        const scored = runLog.allResults.filter(r => r.score !== null);
        // Convergence check
        if (spread < campaign.convergence_threshold && priorRuns.length >= 2) {
            return {
     ...
     */
    buildRunContext(campaign, priorRuns, arms) {
        const parts = [];
        ...
        if (campaign.hypothesis)
            parts.push(`Hypothesis: ${campaign.hypothesis}`);
    ...
    getCampaignRuns(campaignId) {
        const rows = this.db.prepare(`SELECT * FROM campaign_runs WHERE campaign_id = ? ORDER BY sequence_order ASC`).all(campaignId);
        return rows.map(r => ({
