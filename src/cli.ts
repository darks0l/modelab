...
import type { ResearchGoal, ModelabConfig } from './types.js';
...
async function cmdRun(args: string[]) {
  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i] === '--goal' && args[i + 1]) goalText = args[++i];
    else if (args[i] === '--iterations' && args[i + 1]) maxIterations = parseInt(args[++i], 10);
    else if (args[i] === '--threshold' && args[i + 1]) qualityThreshold = parseFloat(args[++i]);
    else if (args[i] === '--arms' && args[i + 1]) numArms = parseInt(args[++i], 10);
  }
