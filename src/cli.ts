...
import { routeTaskV2, formatRoutingDecisionV2, formatModelProfile, type TaskType } from './routing_v2.js';
...
function cmdProfile(args: string[]) {
  ...
  if (dbProfile) {
    console.log(formatModelProfile({
      ...
      strengths: dbProfile.strengths as TaskType[],
      weaknesses: dbProfile.weaknesses as TaskType[],
...
function extractCampaignArg(args: string[], key: string): string | undefined {
 ...
 */
async function cmdCampaignNew(args: string[]) {
  ...
  if (campaign.hypothesis) console.log(`   Hypothesis: ${campaign.hypothesis}`);
 ...
 */
async function cmdCampaignRun(args: string[]) {
 ...
 */
async function cmdCampaignStatus(args: string[]) {
  const campaignId = args.find(a => !a.startsWith('--'));
  if (!campaignId) {
    const config = loadConfig();
    const mgr = new CampaignManager(config);
    const statusFilter = extractCampaignArg(args, '--status') as any;
 ...
 */
async function cmdCampaignReport(args: string[]) {
  const campaignId = args.find(a => !a.startsWith('--'));
  if (!campaignId) {
 ...
 */
async function cmdCampaignSynthesize(args: string[]) {
  const campaignId = args.find(a => !a.startsWith('--'));
  if (!campaignId) {
 ...
 */
async function cmdCampaign(args: string[]) {
  const sub = args[0];
  switch (sub) {
    case 'new':         cmdCampaignNew(args.slice(1));      break;
    case 'run':         cmdCampaignRun(args.slice(1));       break;
    case 'status':      cmdCampaignStatus(args.slice(1));   break;
    case 'report':      cmdCampaignReport(args.slice(1));  break;
    case 'synthesize':  cmdCampaignSynthesize(args.slice(1)); break;
    case 'pause': {
      const config = loadConfig();
      const mgr = new CampaignManager(config);
      const id = args[1];
      if (!id) { console.error('Usage: modelab campaign pause <campaign_id>'); process.exit(1); }
      const updated = mgr.pauseCampaign(id);
      if (!updated) { console.error(`❌ Campaign not found: ${id}`); process.exit(1); }
    ...
    case 'resume': {
      ...
      const mgr = new CampaignManager(config);
      const id = args[1];
      if (!id) { console.error('Usage: modelab campaign resume <campaign_id>'); process.exit(1); }
    ...
    case 'delete': {
      ...
      const mgr = new CampaignManager(config);
      const id = args[1];
      if (!id) { console.error('Usage: modelab campaign delete <campaign_id>'); process.exit(1); }
    ...
    case 'self': {
      const config = loadConfig();
      const mgr = new CampaignManager(config);
      const existing = mgr.listCampaigns('running').find(c =>
        c.question === SELF_IMPROVEMENT_CAMPAIGN.question
      );
      if (existing) {
    ...
    case 'self-run': {
      const config = loadConfig();
      const mgr = new CampaignManager(config);
      let campaign = mgr.listCampaigns('running').find(c =>
        c.question === SELF_IMPROVEMENT_CAMPAIGN.question
      );
      if (!campaign) {
      ...
      try {
        ...
        console.log(`   Belief: ${synthesis.belief_change} | Confidence: ${(synthesis.confidence * 100).toFixed(0)}%`);
        if (synthesis.next_run_recommendation) console.log(`   Next: ${synthesis.next_run_recommendation}`);
