...
export class ExperimentMemory {
  ...
  log(result: ExperimentResult, runId: string, goalId: string): void {
    ...
    stmt.run(
      ...
      result.output,
      result.armId.split(':')[0], // model key embedded in arm id
      result.durationMs,
      result.iteration,
      result.timestamp,
      result.notes ?? null
  ...
  getHistory(goalId?: string): ExperimentResult[] {
    ...
    const rows = goalId
      ? (this.db.prepare(sql).all(goalId) as DbRow[])
  ...
  getBest(goalId: string): ExperimentResult | null {
    const row = this.db.prepare(
      `SELECT * FROM experiments WHERE goal_id = ? AND score IS NOT NULL
  ...
  getTotalSpend(goalId?: string): number {
    ...
    const row = goalId
      ? this.db.prepare(sql).get(goalId) as { total: number | null }
      : this.db.prepare(sql).get() as { total: number | null };
    return row?.total ?? 0;
  }
