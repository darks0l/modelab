...
export class ExperimentMemory {
  ...
  summarize(
    ...
    const allScores = [...priorForGoal, best?.score ?? null].filter((s): s is number => s !== null);
    ...
    const armFamilyScores = new Map<string, number[]>();
  ...
  getContextForIteration(goalId: string, runId: string, iter: number): IterationContext {
    ...
    const armModelScores = new Map<string, { model: string; score: number; temp?: number }[]>();
    ...
    for (const [armFamily, entries] of armModelScores) {
      ...
      for (const e of entries) {
        ...
        if (e.temp !== undefined) (byTemp.get(String(e.temp)) ?? []).push(e.score);
      }
      const bestModel = [...byModel.entries()].sort((a, b) =>
        b[1].reduce((s, v) => s + v, 0) / b[1].length - a[1].reduce((s, v) => s + v, 0) / a[1].length
      )[0];
      const bestTemp = byTemp.size > 0
        ? [...byTemp.entries()].sort((a, b) =>
            b[1].reduce((s, v) => s + v, 0) / b[1].length - a[1].reduce((s, v) => s + v, 0) / a[1].length
          )[0]
        : null;
      if (bestModel) {
        ...
        armModelAdvice.push(
          `**${armFamily}:** prefer ${bestModel[0]} (avg score ${avgScore.toFixed(1)}/10)${bestTemp ? `, temperature ${bestTemp[0]} (avg ${(bestTemp[1].reduce((s, v) => s + v, 0) / bestTemp[1].length).toFixed(1)}/10)` : ''}`
        );
      }
    }
    ...
    if (priorSummaries.length > 0 || armModelAdvice.length > 0 || priorRunLessons.length > 0) {
      ...
      if (armModelAdvice.length > 0) {
        sections.push(
          '## Per-Arm Model Preferences (learned from history)',
          ...armModelAdvice
        );
      }
      ...
      if (avgScore !== null) guidanceLines.push(`Average score across prior arms: ${avgScore.toFixed(1)}/10.`);
      if (armModelAdvice.length > 0) guidanceLines.push(`Model preferences: ${armModelAdvice.slice(0, 3).join('; ')}.`);
      if (avoidAdvice) guidanceLines.push(`Performance warnings:${avoidAdvice}`);
      if (latencyAdvice) guidanceLines.push(`Latency insight:${latencyAdvice}`);
  ...
  private _latencyStatsForRun(runId: string): RunLatencyStats {
    const rows = this.db.prepare(
      `SELECT latency_ms, arm_id FROM experiments WHERE run_id = ? AND latency_ms > 0`
    ).all(runId) as { latency_ms: number; arm_id: string }[];
    if (rows.length === 0) {
  ...
  getModelInsights(): ModelInsight[] {
    ...
    for (const run of runSummaries) {
      ...
      if (question) taskTypeMap.set(run.goalId, _inferTaskType(question));
    }

    // Group by (armFamily, taskType, temperature)
    const groups = new Map<string, {
      armFamily: string; provider: string; model: string;
      taskType: string; temperature: number | null;
      scores: number[]; latencies: number[]; costs: number[];
      bestScore: number | null; wins: number; total: number; lastUsed: string | null;
    }>();
    ...
    for (const r of results) {
      ...
      if (r.latencyMs > 0) g.latencies.push(r.latencyMs);
      g.costs.push(r.costUsd);
      if (g.bestScore === null || r.score > g.bestScore) g.bestScore = r.score;
      if (g.lastUsed === null || r.timestamp > g.lastUsed) g.lastUsed = r.timestamp;
      g.total++;
    }

    // Win rate: per-run best arm
    const runBestMap = new Map<string, { armFamily: string; score: number }>();
    for (const r of results) {
      if (r.score === null) continue;
    ...
    for (const g of groups.values()) {
      ...
      let verdict = '';
      if (avgScore >= 8) verdict = `Strong performer (avg ${avgScore.toFixed(1)}/10, n=${g.scores.length})`;
      else if (avgScore >= 6) verdict = `Solid choice (avg ${avgScore.toFixed(1)}/10, n=${g.scores.length})`;
      else if (avgScore >= 4) verdict = `Below average (avg ${avgScore.toFixed(1)}/10, n=${g.scores.length})`;
      else verdict = `Poor results (avg ${avgScore.toFixed(1)}/10, n=${g.scores.length})`;
      if (winRate > 0.3) verdict += ` · ${(winRate * 100).toFixed(0)}% run wins`;
      if (avgLatency !== null && avgLatency < 2000) verdict += ` · fast TTFT ${Math.round(avgLatency)}ms`;
  ...
  getAggregateStats(goalId?: string): AggregateStats {
    ...
    // Per-model arm count
    const armsByModel: Record<string, number> = {};
    for (const r of results) {
    ...
    const runIterationCounts = new Map<string, number>();
    for (const r of results) {
  ...
  private _extractQuestionFromReport(report: string): string | null {
...
function _inferTaskType(question: string): string {
  if (CODE_KEYWORDS.test(question)) return 'coding';
  if (REASON_KEYWORDS.test(question)) return 'reasoning';
...
export function calcLatencyStats(results: ExperimentResult[]): RunLatencyStats {
